...
function MicAccessTool(o) {
  (this.init = o || {
    ...
    this.initialApp();
  const observer = new MutationObserver(this.initialApp);
  const targetNode = document; // Change this to the element you want to observe

  // Configuration options for the observer
  const config = {
    childList: true, // Watch for additions and removals of child elements
...
(MicAccessTool.prototype.checkLanguageBox = function () {
  ...
  (MicAccessTool.prototype.fontsChange = function (o) {
    ...
    if ("mic-toolbox-fonts-up" === this.id) {
      ...
      A = (1.1 * A).toFixed(2);
    } else if ("mic-toolbox-fonts-down" === this.id) {
      if (A <= 1)
        return (
